...
import { IndustryAnalyzeSchema } from '@/lib/validation';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 输入验证
    const parseResult = IndustryAnalyzeSchema.safeParse(body);
    ...
    const { mbtiType, industryId, industryName } = parseResult.data;

    const mbtiInfo = getMBTIInfo(mbtiType);
    const industryInfo = getIndustryInfo(industryId);

    const userMessage = `我想了解我的${mbtiType}（${mbtiInfo.name}）人格在${industryName || '目标'}行业的发展前景和建议。\n\n我的MBTI特质：${mbtiInfo.description}\n\n目标行业特点：${industryInfo}`;
